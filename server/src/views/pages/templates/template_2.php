...
<div class="container-fluid" id="template_2-container">
    <header class="header">
        <div class="row">
            <h1 class="bg-dark text-light m-2" id="header-content-title"><?= $params["trip"]["title"] ?></h1>
            <div class="col-lg-6 header-image" style="background: url('../../../public/images/<?= unserialize($params["trip"]["images"])[0] ?>') center center; background-size: cover;"></div>
            <div class="col-lg-6 header-content d-flex align-items-center justify-content-center flex-column">
                <p class="bg-light border rounded p-5" id="content-description">
                    <?= $params["trip"]["description"] ?>
                </p>
            </div>
        </div>
    </header>
    <?php if (!empty($params["tripContents"][0])) : ?>
        <div class="mb-5 row first-row">
            ...
            <div class=" image first-image" style="background: url('../../../public/images/<?= unserialize($params["trip"]["images"])[1] ?? 'forest.jpg' ?>') no-repeat center center; background-size: cover;"></div>
        </div>
    <?php endif ?>
    <br><br>
    <?php if (!empty($params["tripContents"][1])) : ?>
        <div class="row second-row mt-5 text-light bg-dark">
            ...
            <div class=" col-lg-5 second-image image" style="background: url('../../../public/images/<?= unserialize($params["trip"]["images"])[2] ?? 'forest.jpg' ?>') center center; background-size: cover;"></div>
        </div>
    <?php endif ?>
    <br><br><br>
